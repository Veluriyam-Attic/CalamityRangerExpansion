...
namespace CalamityRangerExpansion.Content.Arrows.DPreDog.UelibloomArrow
{
    public class UelibloomArrowLight : ModProjectile, ILocalizedModType
    {
        ...
        private static Color ShaderEndColor = Color.LightGreen; // 着色器结束颜色，设置为浅绿色
        ...
        public override void SetDefaults() // 设置弹幕的默认值
        {
            ...
            Projectile.DamageType = DamageClass.Ranged; // 设置弹幕为远程伤害类型
            ...
            Projectile.penetrate = -1; // 弹幕可以穿透无限次
            Projectile.usesLocalNPCImmunity = true; // 使用本地NPC无敌机制
            Projectile.localNPCHitCooldown = -1; // 设置NPC的无敌冷却时间为-1（立即生效）
        }
        ...
        //public override void OnHitPlayer(Player target, Player.HurtInfo info) // 当弹幕击中玩家时的行为
        //{
        //    target.AddBuff(ModContent.BuffType<HolyFlames>(), 90); // 给玩家添加HolyFlames效果（已注释掉）
        //}

        //public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) // 当弹幕击中NPC时的行为
        //{
        //    if (Projectile.damage > 1 && Projectile.numHits < 1) // 如果弹幕伤害大于1且命中次数小于1
        //        Projectile.damage = (int)(Projectile.damage * 0.8f); // 减少弹幕伤害

        //    target.AddBuff(ModContent.BuffType<HolyFlames>(), 90); // 给目标NPC添加HolyFlames效果
        //    Projectile.timeLeft = 80; // 设置弹幕剩余时间为80
        //}

        #endregion
        private float PrimitiveWidthFunction(float completionRatio, Vector2 vexpos) // 计算弹幕宽度变化
        {
        ...
        private Color PrimitiveColorFunction(float completionRatio, Vector2 vexpos) // 计算弹幕颜色变化
        {

...
{
    internal class PEJupiterSPIT : ModProjectile, ILocalizedModType
    {
        public new string LocalizationCategory => "Weapons.DPreDog";
        public override string Texture => "CalamityMod/Projectiles/InvisibleProj";

        public static int MaxUpdate = 5; // 定义一个静态变量，表示弹幕每次更新的最大次数
        private int Lifetime = 220; // 定义弹幕的生命周期为x

        // 木星颜色
        private static Color ShaderColorOne = new Color(176, 123, 89); // 着色器颜色1，设置为浅棕色（木星表面云层）
        private static Color ShaderColorTwo = new Color(242, 66, 54);  // 着色器颜色2，设置为红色（木星的大红斑）
        private static Color ShaderEndColor = new Color(245, 197, 128); // 着色器结束颜色，设置为浅橙色（木星带状云）
        ...
        public override void SetDefaults() // 设置弹幕的默认值
        {
            ...
            Projectile.localNPCHitCooldown = 5; // 设置NPC的无敌冷却时间为5（立即生效）
        }
